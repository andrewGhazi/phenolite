# This version uses cmdstanr to update the lines
...
server <- function(input, output) {
  ...
  output$combinedPlot <- renderPlot({ 
    ...
    est_df = fit$draws(c("ell", "wk_sigma", "intercept", 
                         "wkv",
                         "yrwkv"), 
                       format = 'data.frame') |> 
      qDT() |> 
    ...
    ell = est_df$value[1]
    wk_sigma = est_df$value[2]
    intercept = est_df$value[3]
    
    eff = ss(est_df, seq_len(fnrow(est_df) - 3) + 3)
    wkv_fit = ss(eff, seq_len(52))$value
    yrwkv_fit = ss(eff, seq_len(dl$N) + 52)$value
